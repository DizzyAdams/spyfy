...
// Now renders the REAL creative: a <video> (autoplay/muted/loop) when a video
// URL is present, an <img> when a photo URL is present, and falls back to the
// branded gradient when neither is available.
type OfferCreativeProps = {
  ...
  videoUrl?: string; // direct mp4/webm — plays inline instead of a static poster
...
export function OfferCreative({
  ...
  const Icon = isVideo || hasVideo ? Video : ImageIcon;

  // When a real video is present we always treat the surface as video.
  // If the media fails to load we gracefully fall back to the gradient
  // (and, for video, to the static poster image when available).
  const showVideo = hasVideo && !videoError;
  const showImage = !showVideo && hasImage && !imgError;
  ...
  return (
    ...
    >
      ...
      {/* Real creative — video (plays inline) or image, sits above the gradient */}
      {showVideo && (
        <video
      ...
      <span
        ...
        aria-label={`Formato: ${showVideo ? "Vídeo" : "Imagem"}`}
      >
        <Icon size={12} className="text-accent" aria-hidden />
        <span className="capitalize">{showVideo ? "Vídeo" : "Imagem"}</span>
      </span>

      {/* Center play affordance for video — decorative only (it already plays inline) */}
      {showVideo && (
